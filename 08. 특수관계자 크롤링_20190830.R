...
### (사전정의2) 각 테이블 필요성 결정해주는 함수 정의 - 각 테이블이 해당하는 연도 판별 함수_일단 case1만!
...
list_case_fun <- function(tmp_table, col_name, start_c){
  ...
  if(!(all(str_detect(paste(col_name_new, collapse = ""), c("당기", "전기"))) | all(str_detect(paste(col_name_new, collapse = " "), c("\\(당\\)기", "\\(전\\)기"))))){tmp_case <- "case1"
  ...
  for(i in 1:length(list_df)){
    if(list_TF[i] == TRUE){
      ...
      if(tmp_case == "case1"){
        ...
        if(!(str_detect(tmp[real_r,2], "기업")[1] %in% c(NA, FALSE))){
          # row 이름 명명하기
        ...
        # 숫자만 있는 테이블로 만들기
        unit <- list_unit_fun(list_df, i, get_tab(path, 2))
        ...
        # }
        colnames(tmp) <- text_clean(ifelse(grepl("당", col_name), "당기", ifelse(grepl("전", col_name), "전기", col_name)))
        tmp <- as.data.frame(lapply(tmp, function(x) text_clean(x)))
        if(cate_case == 2){
...
n <- length(n_list[[1]])
write.xlsx(get(n_list[[1]][1]), file = "example_17.xlsx", sheetName = "sheet1")
name_tmp <- paste0("sheet", 2:n)
for (j in 2:n){
  write.xlsx(get(n_list[[1]][j]), file = "example_17.xlsx", sheetName = name_tmp[j-1], append=TRUE)
}
