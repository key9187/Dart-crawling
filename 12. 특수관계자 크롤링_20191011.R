...
# path <- as.character(kospi200$링크[131]); index = 131
# dart_get(as.character(kospi200$링크[3]), 3)
# 확인필요: // 추후확인: 신한금융지주회사, 유니드, 삼성전기
### dart_get 함수 
...
n <- length(n_list[[1]])
write.xlsx(get(n_list[[1]][1]), file = "12. example_1004(40).xlsx", sheetName = "sheet1")
name_tmp <- paste0("sheet", 2:n)
for (j in 2:n){
  write.xlsx(get(n_list[[1]][j]), file = "12. example_1004(40).xlsx", sheetName = name_tmp[j-1], append=TRUE)
}
