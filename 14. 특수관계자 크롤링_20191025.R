...
list_start_fun <- function(tmp_table){
  ...
  stand_n <- ifelse(nrow(tmp_table)>=5, 5, nrow(tmp_table))
...
# str_detect(c(colnames(list_df[[9]]), as.character(list_df[[9]])), paste0(tmp_list_f, collapse = "|"))
# str_extract_all(c(colnames(list_df[[3]]), as.character(list_df[[3]]))[1], paste0(tmp_list_f, collapse = "|"))
...
colname_clean <- function(tmp_table, tmp_r){
  ...
  if(sum(c("당기", "전기") %in% str_extract(colnames(tmp_colname_table), paste0(c("당기", "전기"), collapse="|"))) == 1){
...
# path <- as.character(kospi200$링크[184]); index = 184
# 확인필요: 18 30 / 70까지 확인!
# 9번 특이케이스!
### dart_get 함수 
dart_get <- function(path, index){
  ...
  for(i in 1:length(list_df)){
    if(list_TF[i] == TRUE){
      ...
      # tmp <- tmp[,!str_detect(colnames(tmp), "거래내용")]   #향후수정필요(9번)
      ### 2017, 2018 -> 전기, 당기
  ...
  if(length(which(list_case %in% c("case1", "case1_3"))) %% 2 == 1){
    for(jj in 1:length(list_case)){
      ...
      if( (list_case[jj] == "case1") & (any(str_detect(rbind(colnames(list_df[[jj]]),list_df[[jj]]), 
                                                       paste(c("자금거래", "자금대여", "자금.대여", "배당금", "당기말금액"), collapse = "|")))) ){
        list_case[jj] <- "case4"}
    }
    if(length(which(list_case %in% c("case1", "case1_3"))) %% 2 == 1){
...
error_check_fun(39)   ### '39_(주)엘지유플러스‘ -> 확인완료(74,11)
...
error_check_fun(47)   ### '47_(주)우리은행‘ -> 확인완료(88,18)
error_check_fun(48)   ### '48_(주)유니드‘ -> 확인완료(27,20)
...
error_check_fun(56)   ### '56_(주)카카오‘ -> 확인완료(125,17)
...
error_check_fun(120)   ### '120_삼성물산(주)‘ -> 확인완료(40,14)
...
error_check_fun(184)   ### '184_한일홀딩스(주)‘ -> 확인완료(32,22)
error_check_fun(185)   ### '185_한전케이피에스(주)‘ -> 확인완료(46,20)
error_check_fun(186)   ### '186_한화생명보험(주)‘ -> 확인완료(80,43)
error_check_fun(187)   ### '187_한화에어로스페이스(주)‘ -> 확인완료(79,19)
...
### 테이블생성
koreagas_table <- error_check_fun(169, TRUE)   # '169_한국가스공사(주)‘
koreaelec_table <- error_check_fun(172, TRUE)   # '172_한국전력공사(주)‘
koreaelecpower_table <- error_check_fun(173, TRUE)   # '173_한국전력기술(주)‘
kolmar_table <- error_check_fun(174, TRUE)   # '174_한국콜마(주)‘
koreatire_table <- error_check_fun(175, TRUE)   # '175_한국타이어(주)‘
koreatireworld_table <- error_check_fun(176, TRUE)   # '176_한국타이어월드와이드(주)‘
koreainvest_table <- error_check_fun(177, TRUE)   # '177_한국투자금융지주(주)‘
koreaaero_table <- error_check_fun(178, TRUE)   # '178_한국항공우주산업(주)‘
hanmisience_table <- error_check_fun(179, TRUE)   # '179_한미사이언스(주)‘
hanmi_table <- error_check_fun(180, TRUE)   # '180_한미약품(주)‘
hanse_table <- error_check_fun(181, TRUE)   # '181_한세실업(주)‘
hanon_table <- error_check_fun(182, TRUE)   # '182_한온시스템(주)‘
hanolbio_table <- error_check_fun(183, TRUE)   # '183_한올바이오파마(주)‘
hanilhol_table <- error_check_fun(184, TRUE)   # '184_한일홀딩스(주)‘
hankps_table <- error_check_fun(185, TRUE)   # '185_한전케이피에스(주)‘
hanhwa_table <- error_check_fun(186, TRUE)   # '186_한화생명보험(주)‘
hanhwaair_table <- error_check_fun(187, TRUE)   # '187_한화에어로스페이스(주)‘
hanhwache_table <- error_check_fun(188, TRUE)   # '188_한화케미칼(주)‘
hyundaecon_table <- error_check_fun(189, TRUE)   # '189_현대건설(주)‘
hyundaeglo_table <- error_check_fun(190, TRUE)   # '190_현대글로비스(주)‘
hyundaero_table <- error_check_fun(191, TRUE)   # '191_현대로템(주)‘
hyundaemobis_table <- error_check_fun(192, TRUE)   # '192_현대모비스(주)‘
hyundaeele_table <- error_check_fun(193, TRUE)   # '193_현대엘리베이터(주)‘
hyundaewi_table <- error_check_fun(194, TRUE)   # '194_현대위아(주)‘
hyundaecar_table <- error_check_fun(195, TRUE)   # '195_현대자동차(주)‘
hyundaesteel_table <- error_check_fun(196, TRUE)   # '196_현대제철(주)‘
hyundaeheavy_table <- error_check_fun(197, TRUE)   # '197_현대중공업(주)‘
hyundae_table <- error_check_fun(198, TRUE)   # '198_현대중공업지주(주)‘
hyundaeinsu_table <- error_check_fun(199, TRUE)   # '199_현대해상화재보험(주)‘
hyuseong_table <- error_check_fun(200, TRUE)   # '200_휴성중공업(주)‘
huchems_table <- error_check_fun(201, TRUE)   # '201_휴켐스(주)‘
...
n <- length(n_list[[1]])
# 결과저장
write.xlsx(get(n_list[[1]][1])[1], file = "14. example_1025(31).xlsx", sheetName = "sheet1")
name_tmp <- paste0("sheet", 2:n)
for (j in 2:n){
  write.xlsx(get(n_list[[1]][j])[1], file = "14. example_1025(31).xlsx", sheetName = name_tmp[j-1], append=TRUE)
}
# 결과 except 저장
...
write.xlsx(get(n_list[[1]][list_number[1]])[[2]], file = "14. example_1025(31)_except.xlsx", sheetName = "sheet1")
name_tmp <- paste0("sheet", 2:length(list_number))
for (j in 2:length(list_number)){
  write.xlsx(get(n_list[[1]][list_number[j]])[[2]], file = "14. example_1025(31)_except.xlsx", sheetName = name_tmp[j-1], append=TRUE)
}
