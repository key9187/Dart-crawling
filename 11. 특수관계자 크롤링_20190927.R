...
get_startend <- function(url, case_tmp){
  ...
  text <- subset(text[[1]], (nchar(text[[1]]) <= 20) & (as.numeric(str_extract(text[[1]], "[0-9]*")) > 5) )
...
list_case_fun <- function(tmp_table, col_name, start_c, tmp_list_t){
  ...
  if(tmp_case == "case1"){
    if(any(str_detect(tmp_table[,(start_c-1)], paste(tmp_list_t_new, collapse = '|'))) == TRUE) {tmp_case <- "case1_3"}
  }
...
colname_clean <- function(tmp_table, tmp_r){
  ...
  tmp_colname_table <- as.data.frame(lapply(tmp_colname_table, function(x) gsub("\\.","",gsub("[0-9]","",x))), stringsAsFactors = F)
  ...
  for(j in 1:nrow(tmp_colname_table)){
    ...
    # tmp_colname_list[[j]] <- tmp_j[!(str_detect(tmp_j, paste(c("구분", "회사명", "기업", "특수관계자", "계정과목명", "유형",
    #                                                            "회사의명칭", "값없음", "계정과목", "알수없음", "회사", "관계"),
...
# path <- as.character(kospi200$링크[90]); index = 90
  ...
  for(i in 1:length(list_df)){
    if(list_TF[i] == TRUE){
      ...
      sub_name_tmp <- c("종속기업", "관계기업", "지배기업", "기타특수관계기업",
                        ...
                        "종속,관계기업", "대규모기업집단", "신규설립으로인한현금출자및유상증자")
...
n <- length(n_list[[1]])
write.xlsx(get(n_list[[1]][1]), file = "example_0927(10+40).xlsx", sheetName = "sheet1")
name_tmp <- paste0("sheet", 2:n)
for (j in 2:n){
  write.xlsx(get(n_list[[1]][j]), file = "example_0927(10+40).xlsx", sheetName = name_tmp[j-1], append=TRUE)
}


############################## step 5. column name 정제 & 리스트 ##################################
n_list <- list(ls(pattern = "table"))
col_namelist <- c()
...
write.csv(col_namelist_update1, file="col_namelist.csv")
